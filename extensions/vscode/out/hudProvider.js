...
class HudProvider {
    ...
    getChildren() {
        ...
        const todosItem = new vscode.TreeItem(`Todos in-progress: ${inProgress.length}`);
        ...
        if (hud.checks?.ci_status) {
            ...
            ci.iconPath = new vscode.ThemeIcon(hud.checks.ci_status === "passing" ? "pass" : "error");
            items.push(ci);
        }
        if (hud.cost?.session_usd !== undefined) {
            const cost = new vscode.TreeItem(`Cost: $${hud.cost.session_usd.toFixed(4)}`);
        ...
        if (risk) {
            ...
            riskItem.iconPath = new vscode.ThemeIcon(flagCount > 0 ? "warning" : "shield");
            if (flagCount > 0) {
...
exports.HudProvider = HudProvider;
