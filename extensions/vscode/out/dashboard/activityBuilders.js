...
exports.buildFileActivity = buildFileActivity;
exports.buildRecentAgents = buildRecentAgents;
exports.buildSessionAgents = buildSessionAgents;
exports.buildSessionAgentActivity = buildSessionAgentActivity;
exports.sessionNick = sessionNick;
...
function buildFileActivity(events, limit) {
    const fileMap = new Map();
    for (const ev of [...events].reverse()) {
        if (!ev.file && !ev.cmd && ev.tool !== "Skill")
            continue;
...
function buildRecentAgents(events, now = Date.now()) {
    const agentMap = new Map();
    for (const ev of events) {
        const secAgo = Math.floor((now - new Date(ev.ts).getTime()) / 1000);
        if (ev.tool === "AgentStart") {
            const key = ev.agent_id || ev.label || ev.ts;
            if (secAgo < RECENT_AGENT_MS / 1000) {
                agentMap.set(key, {
                    agentId: key,
                    label: ev.label ?? ev.agent_label ?? "agent",
                    role: ev.role ?? "",
                    skill: ev.skill ?? "",
                    ts: ev.ts,
                    done: false,
            ...
            const key = ev.agent_id || ev.label || "";
...
function buildSessionAgents(events, sessionIdleMs, now = Date.now()) {
    const agentMap = new Map();
    for (const ev of events) {
        if (ev.tool === "AgentStart") {
            const key = ev.agent_id || ev.label || ev.ts;
...
function buildSessionAgentActivity(events, agents) {
    const activityMap = new Map();
    for (const agent of agents)
        activityMap.set(agent.agentId || agent.label, { ...agent, activity: [] });
    for (const ev of [...events].reverse()) {
        const agentId = ev.agent_id || ev.agent || "";
        if (!agentId)
            continue;
        ...
        if (existing) {
            ...
            (entry.activity ?? (entry.activity = [])).push({ file: itemKey, tool: ev.tool, count: 1, lastTs: ev.ts, agentId, agentLabel: label });
        }
        if (ev.ts > entry.ts)
            entry.ts = ev.ts;
