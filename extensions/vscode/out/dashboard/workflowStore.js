...
exports.readWorkflowTranscriptAgents = readWorkflowTranscriptAgents;
exports.readWorkflowPlan = readWorkflowPlan;
exports.readSessionWorkflowState = readSessionWorkflowState;
const fs = require("fs");
const os = require("os");
                        ...
                        for (const rawLine of chunk.split("\n")) {
                            ...
                            try {
                                ...
                                if (e.message?.model && !model)
                                    model = (0, formatters_1.fmtModel)(e.message.model);
                            }
                            catch {
...
function readSessionWorkflowState(root, sessionId, getEventsForRoot) {
    ...
    for (const ev of wfEvents) {
        if (ev.tool === "WorkflowStart") {
            wfStartTs = ev.ts;
            hasWorkflow = true;
            workflowAgentCount = ev.agent_count ?? 0;
            workflowLabel = ev.label ?? "workflow";
        }
    ...
    if (hasRunningTranscriptAgent)
        hasWorkflow = true;
    if (hasWorkflow && transcriptAgents.length > 0 && !hasRunningTranscriptAgent)
        hasWorkflow = false;
