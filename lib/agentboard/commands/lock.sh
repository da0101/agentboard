...
_lock_acquire() {
  ...
  while true; do
    ...
    _resp="$(curl -sf -m 2 -w '\n%{http_code}' -X POST "http://127.0.0.1:${_port}/lock" \
      ...
      -d "{\"file\":\"$_file\",\"provider\":\"$_provider\"}" 2>/dev/null || true)"
    ...
    if [[ "$_code" == "200" ]]; then
      ...
      if (( _first )); then
        say "${C_DIM}Waiting for lock on $_file (held by ${_holder:-unknown})...${C_RESET}"
        _first=0
      fi
...
_lock_release() {
  ...
  curl -sf -m 2 -X DELETE "http://127.0.0.1:${_port}/lock" \
    -H 'Content-Type: application/json' \
    -d "{\"file\":\"$_file\",\"provider\":\"$_provider\"}" >/dev/null 2>&1 || true
...
_lock_list() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    printf '%s\n' "$_resp" | jq -r '.[] | [.file, .holder, (.held_seconds | tostring) + "s", (.queue | join(","))] | @tsv' 2>/dev/null \
